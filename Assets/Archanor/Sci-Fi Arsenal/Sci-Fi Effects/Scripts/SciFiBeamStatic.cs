	...
	public class SciFiArsenalBeamStatic : MonoBehaviour
	{
		...
		void FixedUpdate()
		{
			if (beam)
			{
				...
				if(beamCollider)
				{
					UpdateBeamCollider(transform.position, end, line.startWidth);
				}
			}
		...
		private void CreateBeamCollider()
		{
			if(beamCollider == null)
			{
				GameObject collider = new GameObject("BeamCollider");

				collider.transform.parent = beam.transform;
				collider.transform.localPosition = Vector3.zero;

				beamCollider = collider.AddComponent<BoxCollider>();
				beamCollider.isTrigger = true;
			}
		}

		private void UpdateBeamCollider(Vector3 startPoint, Vector3 endPoint, float width)
		{
			...
			beamCollider.transform.LookAt(endPoint);

			float distance = Vector3.Distance(startPoint, endPoint);

			beamCollider.size = new Vector3(width, width, distance);
		}
